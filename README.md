...
## Webová vizualizace (`docs/index.html`)

Po každém běhu se vygeneruje statická stránka s grafem hodnoty portfolia v čase, aktuální
alokací a posledním briefingem — hostovaná zdarma na **GitHub Pages** (žádný server navíc).

**Jednorázové zapnutí:** na GitHubu **Settings → Pages → Build and deployment → Source:
„Deploy from a branch"**, branch `main`, složka `/docs` → Save. Za pár minut poběží na
`https://<tvůj-github-username>.github.io/<repo>/`. Stránka nemá heslo (GitHub Pages u
veřejného repa jinak zdarma nejde) — neobsahuje žádné klíče, jen ceny/texty briefingu.
